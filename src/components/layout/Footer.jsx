...
export default function Footer() {
  return (
    <footer className="bg-gray-900 dark:bg-dark-bg text-gray-300 pt-16 pb-8">
      <div className="section-wrapper grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10 mb-12">
        ...
        <div>
          ...
          <p className="mt-4 text-sm leading-relaxed text-gray-400">
            Melbourne's most trusted interior and exterior painters.
            Premium results, transparent pricing, guaranteed workmanship.
          </p>
        ...
        <div>
          ...
          <ul className="space-y-2">
            {['/', '/services', '/portfolio', '/reviews', '/blog', '/contact'].map((href, i) => {
              const labels = ['Home', 'Services', 'Portfolio', 'Reviews', 'Blog', 'Contact'];
              return (
                <li key={href}>
                  <Link to={href}
        ...
        <div>
          ...
          <ul className="space-y-4">
            <li>
              <a href="tel:0391234567" className="flex items-center gap-3 text-gray-400
            ...
            <li>
              <a href="mailto:hello@melbournepropainters.com.au"
                className="flex items-start gap-3 text-gray-400 hover:text-brand-400 transition-colors">
                <Mail size={16} className="text-brand-500 flex-shrink-0 mt-0.5" />
                <span className="text-sm break-all">hello@melbournepropainters.com.au</span>
              </a>
            </li>
            <li className="flex items-start gap-3 text-gray-400">
              <MapPin size={16} className="text-brand-500 flex-shrink-0 mt-0.5" />
              <span className="text-sm">Serving All Melbourne Suburbs<br />VIC 3000, Australia</span>
            </li>
          </ul>
        </div>
      </div>
      ...
      {/* Bottom bar */}
      <div className="section-wrapper border-t border-gray-800 pt-6 flex flex-col sm:flex-row
        items-center justify-between gap-3 text-xs text-gray-500">
        <p>© {new Date().getFullYear()} Melbourne Pro Painters. All rights reserved.</p>
        <p>Lic. No. DB-12345 | ABN 12 345 678 901</p>
      </div>
    </footer>
  );
}
